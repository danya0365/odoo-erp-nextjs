...
export interface DeliverLineView {
  id: string;
  productName: string;
  remaining: number; // scaled
  uom: string;
}
...
export function DeliverForm({
  ...
  orderId: string;
  lines: DeliverLineView[];
}) {
  const [state, formAction, pending] = useActionState<FormState, FormData>(
    deliverSalesOrderAction,
    {},
  );
  const pending_lines = lines.filter((l) => l.remaining > 0);
  if (pending_lines.length === 0) return null;
    ...
    <form action={formAction} className="flex flex-col gap-3">
      ...
      <div>
        <Button type="submit" disabled={pending}>
          {pending ? "กำลังบันทึก…" : "บันทึกการส่งของ"}
        </Button>
      </div>
    </form>
  );
