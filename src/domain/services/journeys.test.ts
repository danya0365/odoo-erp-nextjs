...
test("ข้อมูลจริง: real-world journeys ทำครบทั้งหมดแล้ว (ไม่มี step ที่ยังไม่มี)", () => {
  // ไล่เก็บครบทั้ง 12 real-world journeys แล้ว → ไม่มี step สถานะ missing เหลือ
  const sum = gapSummary(JOURNEYS);
  assert.equal(sum.missing, 0, "ไม่ควรเหลือฟีเจอร์ที่ยังไม่มี");
  for (const j of realWorldJourneys()) {
    for (const s of j.steps) {
      assert.notEqual(s.status, "missing", `${j.id}: ${s.title} ยัง missing`);
    }
  }
});
