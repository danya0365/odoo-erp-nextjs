---
description: ย้าย memory file ที่เลิกใช้เข้า _archive/ (library) ตาม lifecycle ของ odoo-erp
argument-hint: "[path ของไฟล์ memory ที่จะ archive]"
---

Archive memory file ตาม lifecycle ใน `.claude/memory/MEMORY-GUIDE.md`
ไฟล์เป้าหมาย: **$ARGUMENTS** (ถ้าไม่ได้ระบุ ให้ถามว่าจะ archive อันไหน — อาจรัน `/memory-status` ช่วยดู)
...
5. เพิ่ม 1 แถวใน `.claude/memory/_archive/INDEX.md`: ชื่อไฟล์ · เหตุผลที่ archive · วันที่
   - ถ้าไม่ได้บอกเหตุผล ให้ถามสั้นๆ
6. สรุปว่า archive อะไร และย้ำว่าไฟล์ยัง promote กลับได้ด้วย (ทำกลับด้านของขั้นตอนนี้)
